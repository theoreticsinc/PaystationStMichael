        ...
        <section class="content">
          ...
          <tr>
            <td>Available Slots</td>
              <?php
               $query = "SELECT * FROM carpark.total_slots";
              //echo $query;
          ...
          <tr>
            <?php
              //$query = "SELECT COUNT(ParkerType) as Count, ParkerType, TimeIN FROM `entrance` WHERE TimeIN = CURRENT_DATE() GROUP BY ParkerType";
              $query = "SELECT COUNT(n.ParkerType) as Count, p.ptypename as ParkerType, TimeIN FROM carpark.entrance n INNER JOIN parkertypes.main p ON n.ParkerType = p.parkertype GROUP BY n.ParkerType";
              //echo $query;
              $rev = getdata_inner_join($query); //query for getting the results
              //echo $rev;
              if (is_array($rev)) {
