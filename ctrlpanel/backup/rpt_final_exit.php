    ...
    <div class="content-wrapper">
        ...
        <section class="content">
          ...
          if(isset($_GET['searchbtn1'])){
            ...
            $query = "SELECT * FROM carpark.exit_trans WHERE DATE(DateTimeOUT) BETWEEN '$from $timefrom' AND '$until $timeuntil' GROUP BY ExitID, DateTimeOUT ORDER BY ExitID, DateTimeOUT DESC";
            //echo $query;
        ...
        <tr>
          <td><b>Volume:</td></td>
          <td><?php 
                $query = "SELECT COUNT(DateTimeOUT) as total, SUM(Amount) as amount FROM carpark.exit_trans WHERE DATE(DateTimeOUT) BETWEEN '$from $timefrom' AND '$until $timeuntil'";
          ...
          <?php
          $query = "SELECT * FROM carpark.void_trans WHERE DATE(DateTimeOUT) BETWEEN '$from $timefrom' AND '$until $timeuntil' GROUP BY ExitID, DateTimeOUT ORDER BY ExitID, DateTimeOUT DESC";
            //echo $query;
            $cash = getdata_inner_join($query); //query for getting the results
            ?>
          <table class="table table-border">
        ...
        <tr>
          <td><b>Volume:</td></td>
          <td><?php 
                $query = "SELECT COUNT(DateTimeOUT) as total, SUM(Amount) as amount FROM carpark.void_trans WHERE DATE(DateTimeOUT) BETWEEN '$from $timefrom' AND '$until $timeuntil'";
                $trans = single_inner_join($query);//query for getting the results
                $total = 0;
                $amount = 0;
                $total = $total + $trans['total'];
