...
<!-- /navigation -->
    <div class="content-wrapper">
        ...
        <section class="content">
          ...
          <form method="get">
            <select class="txtfield" name="reports">
              <option value="1">Cash and Card Accountability</option>
              <!--<option value="2">List of Timein/Timeout</option>-->
              <option value="3">Collection Report</option>
              <option value="4">Collection Report per Hour</option>
            </select>
            <button class="btn btn-success" name="search_btn"><i class="fa fa-check"></i> Search</button>
          </form>
        </div>
          <p></p>
          <?php 
          if(isset($_GET['search_btn']) && $_GET['reports'] == '1'){
            $reports = filter($_GET['reports']);
            //Search button for Cash and Card Accountability
          ...
          <?php
            //}
          }elseif(isset($_GET['search_btn']) && $_GET['reports'] == '2'){
          ?>
          <hr>
          <h3>Date Filter</h3>
          <form method="get">
          <input type="hidden" name="reports" class="btn btn-primary" value="2">
          From: <input type="date" name="from" class="txtfield"> <p></p>
          Until: <input type="date" name="until" class="txtfield">
          <p></p>
          <button class="btn btn-danger" name="per_date" value="Generate"><i class="fa fa-check"></i> Generate</button>
          </form>
          
          <?php
          }elseif(isset($_GET['search_btn']) && $_GET['reports'] == '3'){
            //Collection Report
          ?>
          <hr>
          <h3>Date Filter</h3>
          <form method="get">
          <input type="hidden" name="reports" class="btn btn-primary" value="3">
          From: <input type="date" name="from" class="txtfield"> <p></p>
          Until: <input type="date" name="until" class="txtfield">
          <p></p>
          <button class="btn btn-success" name="search_btn2" value="Generate"><i class="fa fa-check"></i> Generate</button>
          </form>
          <?php
          }elseif(isset($_GET['search_btn']) && $_GET['reports'] == '4'){
            //Collection Report per Hour
          ?>
           <hr>
          <h3>Date Filter</h3>
          <form method="get">
          ...
          Until: <input type="date" name="until" class="txtfield">
          <p></p>
          <button class="btn btn-success" name="search_btn3" value="Generate"><i class="fa fa-check"></i> Generate</button>
          </form>
          <?php 
          }
          ?>
...
?>
          ...
          <table class="table table-border">
            ...
            <tr>
              ...
              <td>
                <?php $n_total = $retail['total_r'] + $total1; echo $n_total;?>
              </td>
            </tr>
            <tr>
              <td>VIP parker</td>
              ...
              <td> <?php $n_total = $vip['total_v'] + $total2; echo $n_total;?></td>
            </tr>
            <tr>
              <td>Motorcycles</td>
              <td>
              ...
              <td> <?php $n_total = $motor['total_m'] + $total3; echo $n_total;?></td>
            </tr>
            <tr>
              <td>Delivery Van</td>
              <td>
              ...
              <td> <?php $n_total = $delivery['total_d'] + $total4; echo $n_total;?></td>
            </tr>
          ...
          <!-- Exporting data-->
            <a href="export_account.php?from=<?php echo $_GET['from']?>&until=<?php echo $_GET['until']?>" id = "printPageButton" class="btn btn-danger"><span class="glyphicon glyphicon-circle-arrow-down"></span> Export to Excel/CSV</a>
            <a href="" id = "printPageButton" onClick="window.print();" class="btn btn-primary"><span class="glyphicon glyphicon-paperclip"></span> Export to PDF file</a>
          <!-- End of exporting-->
          <?php
          }
          ?>
          <?php 
          if(isset($_GET['search_btn3'])){
             $from = filter($_GET['from']);
            $until = filter($_GET['until']);

            $query  = "SELECT * FROM entrance WHERE DATE(TimeIN) BETWEEN '$from' AND '$until'";
            $result = single_inner_join($query);//query for getting the results
            echo $result['PlateNumber'];
          ?>
           <table class="table table-border">
            <tr>
              <td>Start Date:</td>
            ...
            <tr>
              ...
              <td>
                <?php 
                  ...
                  $result = single_inner_join($j);//query for getting the results
                  echo $result['total']; 
                ?>
              </td>
              <td>
          ...
          <!-- Exporting data-->
          <a href="export_time.php?from=<?php echo $_GET['from']?>&until=<?php echo $_GET['until']?>" id = "printPageButton" class="btn btn-danger"><span class="glyphicon glyphicon-circle-arrow-down"></span> Export to Excel/CSV</a>
          <a href="" id = "printPageButton" onClick="window.print();" class="btn btn-primary"><span class="glyphicon glyphicon-paperclip"></span> Export to PDF file</a>
          <!-- End of exporting-->
          <?php 
          }
